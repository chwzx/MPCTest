...
func main() {
	// 检查命令行参数
	if len(os.Args) < 2 {
		log.Fatal("Usage: go run main.go <certificate-name>")
	}
	certName := os.Args[1]
	...
	// 使用根证书进行签名
	cert, err := x509.CreateCertificate(randReader, &template, rootCA, &privateKey.PublicKey, rootCAKey)
	if err != nil {
		log.Fatalf("Failed to create certificate: %v", err)
	}
	...
	if err != nil {
		log.Fatalf("Failed to open %s.crt for writing: %v", certName, err)
	}
	defer certOut.Close()
	...
	if err != nil {
		log.Fatalf("Failed to open %s.key for writing: %v", certName, err)
	}
	defer keyOut.Close()
